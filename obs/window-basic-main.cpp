	  ...
	  ui             (new Ui::OBSBasic)
{
	...
	qss.append("QPushButton:checked{image:url(:/res/images/btn_play_normal.png);}");
	qss.append("QPushButton:checked:hover{image:url(:/res/images/btn_play_press.png);}");
	qss.append("QPushButton:!checked{image:url(:/res/images/btn_pause_normal.png);}");
	qss.append("QPushButton:!checked:hover{image:url(:/res/images/btn_pause_press.png);}");
	ui->recordButton->setStyleSheet(qss.join(""));
	ui->recordButton->setCheckable(true);
	ui->recordButton->setChecked(true);
	ui->recordButton->setMinimumSize(40, 40);
	//
...
{
	//QMessageBox::information(NULL, "Title", "contentimage", QMessageBox::Yes | QMessageBox::No, QMessageBox::Yes);
	ui->sources->setVisible(false);
	//SaveProject();

	//ui->streamButton->setEnabled(false);
	//ui->streamButton->setText(QTStr("Basic.Main.Connecting"));
	/*QIcon icon("D:/Program Files (x86)/PolyvLive/ui/btnopen.bmp");
	ui->streamButton->setIcon(icon);
	*/
...
void OBSBasic::RecordingStop(int code)
{
	...
	//2016-7-14
	//test 
	//QMessageBox::information(NULL, "PGY", " todo ", QMessageBox::Yes | QMessageBox::No, QMessageBox::Yes);
	ShowRecordings();
...
void OBSBasic::on_actionWebsite_triggered()
{
	...
	QUrl url = QUrl("http://www.pugongying.com.cn", QUrl::TolerantMode);
	QDesktopServices::openUrl(url);
}
...
//水印操作 
void OBSBasic::on_actionBasic_MainMenu_File_Source_triggered()
{
	bool isOk;
	QString text = QInputDialog::getText(NULL, "input dilog",
	...
	//QMessageBox::information(NULL, "PGY", "to do ?", QMessageBox::Yes | QMessageBox::No, QMessageBox::Yes);
	if (ui->sources->isVisible())
	{
	//MessageBox::information(NULL, "PGY", "setvisibe=false ?", QMessageBox::Yes | QMessageBox::No, QMessageBox::Yes);

		ui->sources->setVisible(false);
	}
	else
	{
		//QMessageBox::information(NULL, "PGY", "setvisible = true ?", QMessageBox::Yes | QMessageBox::No, QMessageBox::Yes);
